...
import { RestrictedAuctionHouse, TestERC721, WETH, TestAccessControl } from "../typechain";
import { deployAccessControl } from "./utils";
...
describe("integration", () => {
  ...
  let media: Media;
  let weth: WETH;
  let auction: RestrictedAuctionHouse;
  ...
  describe("Restricted ETH auction with curator", () => {
    async function run() {
      await media.connect(owner).approve(auction.address, 0);
      await auction
        .connect(owner)
